"""
Persisted transformer sizing runs (EOS-03a, Master Prompt A16).
...
The stateless /electrical/transformer-sizing/calculate route stays a quick
calculation and stores nothing.
"""
...
router = APIRouter(
    prefix="/electrical/transformer-sizing/runs",
    tags=["Electrical Transformer Sizing"],
)


def get_service(db: DatabaseSession) -> TransformerRunService:
    return TransformerRunService(
        CalculationRunRepository(db), ProjectService(ProjectRepository(db))
    )
...
@router.post(
    "",
    response_model=TransformerRunResponse,
    status_code=status.HTTP_201_CREATED,
)
async def create_transformer_run(
    payload: TransformerRunCreateRequest,
    db: DatabaseSession,
    response: Response,
    identity: CurrentSession,
) -> TransformerRunResponse:
    """Calculate a transformer sizing study and persist it as a new run revision."""
...
async def list_transformer_runs(
    db: DatabaseSession,
    identity: CurrentSession,
    ...
    """List the transformer runs of one scope.

    The scope is a project revision, or the runs that belong to no project.
    """
...
async def get_transformer_run(
    run_id: UUID,
    db: DatabaseSession,
    identity: CurrentSession,
) -> CalculationRunDetail:
    """Return one persisted transformer run with its frozen snapshots."""

    service = get_service(db)
    run = await service.get(run_id)
    missing = HTTPException(
        status_code=status.HTTP_404_NOT_FOUND,
        detail=f"Transformer run {run_id} was not found",
    )
    if run is None or run.module_code != TRANSFORMER_MODULE_CODE:
        raise missing
