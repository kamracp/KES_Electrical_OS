...
def _calculate_feeder_result(
    ...
    with localcontext() as context:
        ...
        current_loading_percent = (
            feeder.design_current_a / feeder.rated_normal_current_a * Decimal("100")
        )
        ...
        ct_margin_a = feeder.ct_primary_current_a - feeder.design_current_a
    ...
    if (
        ...
        warnings.append(
            HTPanelWarning(
                code=(HTPanelWarningCode.BREAKING_CAPACITY_MARGIN_LOW),
                message=("HT feeder breaking-capacity margin is below 25 percent."),
            )
        )
    ...
    if (
        ...
        warnings.append(
            HTPanelWarning(
                code=(HTPanelWarningCode.SHORT_TIME_WITHSTAND_MARGIN_LOW),
                message=("HT feeder short-time withstand margin is below 25 percent."),
            )
        )

    if feeder.ct_primary_current_a < feeder.design_current_a * LOW_MARGIN_FACTOR:
        warnings.append(
            HTPanelWarning(
                code=HTPanelWarningCode.CT_RATIO_MARGIN_LOW,
                message=("CT primary-current margin is below 25 percent."),
            )
        )
    ...
    return HTFeederResult(
        ...
        rated_normal_current_a=(feeder.rated_normal_current_a),
        current_loading_percent=_round_value(current_loading_percent),
        prospective_short_circuit_current_ka=(feeder.prospective_short_circuit_current_ka),
        rated_short_circuit_breaking_current_ka=(feeder.rated_short_circuit_breaking_current_ka),
        breaking_capacity_margin_ka=_round_value(breaking_capacity_margin_ka),
        rated_short_time_withstand_current_ka=(feeder.rated_short_time_withstand_current_ka),
        short_time_withstand_margin_ka=_round_value(short_time_withstand_margin_ka),
        short_time_withstand_duration_s=(feeder.short_time_withstand_duration_s),
        rated_peak_withstand_current_ka=(feeder.rated_peak_withstand_current_ka),
        ct_primary_current_a=feeder.ct_primary_current_a,
        ct_secondary_current_a=feeder.ct_secondary_current_a,
        ct_ratio=(
            f"{feeder.ct_primary_current_a.normalize()}/{feeder.ct_secondary_current_a.normalize()}"
        ),
...
def _build_spare_feeder_result(
    ...
    return HTFeederResult(
        ...
        rated_short_circuit_breaking_current_ka=(panel.busbar_short_time_withstand_current_ka),
        breaking_capacity_margin_ka=(panel.busbar_short_time_withstand_current_ka),
        rated_short_time_withstand_current_ka=(panel.busbar_short_time_withstand_current_ka),
        short_time_withstand_margin_ka=(panel.busbar_short_time_withstand_current_ka),
        short_time_withstand_duration_s=(panel.busbar_short_time_duration_s),
        rated_peak_withstand_current_ka=(panel.busbar_peak_withstand_current_ka),
        ct_primary_current_a=panel.busbar_rated_current_a,
        ct_secondary_current_a=Decimal("1"),
        ct_ratio=(f"{panel.busbar_rated_current_a.normalize()}/1"),
...
def calculate_ht_panel_sizing(
    ...
    if not isinstance(
        ...
        raise TypeError("sizing_input must be an HTPanelSizingInput record")
    ...
    with localcontext() as context:
        ...
        maximum_feeder_current_a = max(
            feeder.rated_normal_current_a for feeder in sizing_input.feeders
        )
    ...
    if busbar_loading_percent >= HIGH_LOADING_PERCENT:
        warnings.append(
            HTPanelWarning(
                ...
                message=("Calculated HT busbar loading is at or above 90 percent."),
            )
        )
    ...
    if busbar_loading_percent < LOW_LOADING_PERCENT:
        warnings.append(
            HTPanelWarning(
                ...
                message=(
                    "Calculated HT busbar loading is below 25 percent; review possible oversizing."
                ),
            )
        )
    ...
    if sizing_input.arc_classification_required:
        warnings.append(
            HTPanelWarning(
                code=(HTPanelWarningCode.ARC_CLASSIFICATION_REQUIRED),
                message=(
    ...
    if sizing_input.installation.value == "OUTDOOR" and not sizing_input.remote_operation_required:
        warnings.append(
            HTPanelWarning(
                code=(HTPanelWarningCode.REMOTE_OPERATION_RECOMMENDED),
                message=("Remote breaker operation should be reviewed for outdoor HT switchgear."),
            )
        )
    ...
    return HTPanelSizingResult(
        ...
        maximum_feeder_current_a=_round_value(maximum_feeder_current_a),
