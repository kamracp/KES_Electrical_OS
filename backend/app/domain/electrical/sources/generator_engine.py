...
def _calculate_raw_values(
    ...
    with localcontext() as context:
        ...
        steady_state_demand_kva = (
            sizing_input.steady_state_demand_kw / sizing_input.steady_state_power_factor
        )

        future_steady_state_kva = steady_state_demand_kva * sizing_input.future_growth_factor

        steady_state_required_kva = future_steady_state_kva * sizing_input.design_margin_factor

        transient_additional_kva = (
            sizing_input.transient_step_load_kva * sizing_input.transient_allowance_factor
        )

        transient_required_kva = future_steady_state_kva + transient_additional_kva
        ...
        required_nameplate_capacity_kva = governing_required_kva / combined_derating_factor
    ...
    return _RawGeneratorSizingValues(
        steady_state_demand_kva=(steady_state_demand_kva),
        future_steady_state_kva=(future_steady_state_kva),
        steady_state_required_kva=(steady_state_required_kva),
        transient_additional_kva=(transient_additional_kva),
        transient_required_kva=(transient_required_kva),
        governing_required_kva=(governing_required_kva),
...
def _build_common_warnings(
    ...
    if raw_values.combined_derating_factor < Decimal("1"):
        warnings.append(
            GeneratorSizingWarning(
                code=(GeneratorSizingWarningCode.DERATING_APPLIED),
                message=(
    ...
    if raw_values.transient_required_kva > raw_values.steady_state_required_kva:
        warnings.append(
            GeneratorSizingWarning(
                code=(GeneratorSizingWarningCode.TRANSIENT_REQUIREMENT_GOVERNS),
                message=("The transient or starting-load requirement governs generator selection."),
            )
        )
...
def _build_no_solution_result(
    ...
    warnings = _build_common_warnings(raw_values)

    warnings.append(
        GeneratorSizingWarning(
            code=(GeneratorSizingWarningCode.NO_STANDARD_RATING_AVAILABLE),
            message=(
                "No available generator unit rating satisfies the calculated capacity requirement."
            ),
        )
    )
    ...
    return GeneratorSizingResult(
        ...
        steady_state_demand_kw=(sizing_input.steady_state_demand_kw),
        steady_state_power_factor=(sizing_input.steady_state_power_factor),
        steady_state_demand_kva=_round_capacity(raw_values.steady_state_demand_kva),
        future_growth_factor=(sizing_input.future_growth_factor),
        future_steady_state_kva=_round_capacity(raw_values.future_steady_state_kva),
        design_margin_factor=(sizing_input.design_margin_factor),
        steady_state_required_kva=_round_capacity(raw_values.steady_state_required_kva),
        transient_step_load_kva=(sizing_input.transient_step_load_kva),
        transient_allowance_factor=(sizing_input.transient_allowance_factor),
        transient_additional_kva=_round_capacity(raw_values.transient_additional_kva),
        transient_required_kva=_round_capacity(raw_values.transient_required_kva),
        governing_required_kva=_round_capacity(raw_values.governing_required_kva),
...
def _build_selected_rating_result(
    ...
    with localcontext() as context:
        ...
        spare_derated_capacity_kva = derated_duty_capacity_kva - raw_values.governing_required_kva

        steady_state_loading_percent = (
            raw_values.steady_state_required_kva / derated_duty_capacity_kva * Decimal("100")
        )

    warnings = _build_common_warnings(raw_values)

    if steady_state_loading_percent >= HIGH_LOADING_LIMIT_PERCENT:
        warnings.append(
            GeneratorSizingWarning(
                code=(GeneratorSizingWarningCode.HIGH_LOADING),
                message=("Calculated steady-state generator loading is at or above 90 percent."),
            )
        )

    if steady_state_loading_percent < LOW_LOADING_LIMIT_PERCENT:
        warnings.append(
            GeneratorSizingWarning(
                code=(GeneratorSizingWarningCode.LOW_LOADING),
                message=(
    ...
    status = GeneratorSizingStatus.WARNING if warnings else GeneratorSizingStatus.VALID
    ...
    return GeneratorSizingResult(
        ...
        steady_state_loading_percent=_round_percent(steady_state_loading_percent),
        status=status,
        warnings=tuple(warnings),
    )
...
def calculate_generator_sizing(
    ...
    if not isinstance(
        ...
        raise TypeError("sizing_input must be a GeneratorSizingInput record")
