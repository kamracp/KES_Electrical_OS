...
class LoadCalculationRunService:
    ...
    async def create(
        ...
        if not isinstance(
            ...
            raise TypeError("payload must be a LoadCalculationRunCreate")

        content_hash = self._build_content_hash(payload)

        duplicate_run = await self.repository.get_latest_by_content_hash(content_hash)

        if duplicate_run is not None and duplicate_run.calculation_key == payload.calculation_key:
            raise ValueError("an identical calculation run already exists")

        latest_revision = await self.repository.get_latest_revision(payload.calculation_key)

        revision_number = await self.repository.get_next_revision_number(payload.calculation_key)
        ...
        calculation_run = LoadCalculationRun(
            ...
            input_snapshot=snapshot_data["input_snapshot"],
            result_snapshot=snapshot_data["result_snapshot"],
            assumptions_snapshot=snapshot_data["assumptions_snapshot"],
        ...
        return await self.repository.create(calculation_run)
    ...
    async def get_by_id(
        ...
        calculation_run = await self.repository.get_by_id(run_id)
    ...
    async def list_revision_history(
        ...
        if not normalized_key:
            raise ValueError("calculation_key must not be empty")

        return await self.repository.list_by_calculation_key(normalized_key)
    ...
    async def submit(
        ...
        if not isinstance(
            ...
            raise TypeError("payload must be a LoadCalculationRunSubmit")

        calculation_run = await self.get_by_id(run_id)

        self._ensure_mutable(calculation_run)

        if calculation_run.run_status != CalculationRunStatus.COMPLETED.value:
            raise ValueError("only completed calculation runs can be submitted")

        if calculation_run.approval_status != CalculationApprovalStatus.NOT_SUBMITTED.value:
    ...
    async def approve(
        ...
        if not isinstance(
            ...
            raise TypeError("payload must be a LoadCalculationRunApprove")
    ...
    async def reject(
        ...
        if not isinstance(
            ...
            raise TypeError("payload must be a LoadCalculationRunReject")
    ...
    async def compare(
        ...
        target_run = await self.get_by_id(target_run_id)

        if base_run.calculation_key != target_run.calculation_key:
            raise ValueError("calculation runs must have the same calculation_key")

        audit_base = {
            "scenario": base_run.scenario,
            "run_status": base_run.run_status,
            "approval_status": (base_run.approval_status),
            "engine_version": base_run.engine_version,
            "formula_version": (base_run.formula_version),
            "assumptions_snapshot": (base_run.assumptions_snapshot),
            "warnings_snapshot": (base_run.warnings_snapshot),
            "standards_snapshot": (base_run.standards_snapshot),
        }

        audit_target = {
            "scenario": target_run.scenario,
            "run_status": target_run.run_status,
            "approval_status": (target_run.approval_status),
            "engine_version": (target_run.engine_version),
            "formula_version": (target_run.formula_version),
            "assumptions_snapshot": (target_run.assumptions_snapshot),
            "warnings_snapshot": (target_run.warnings_snapshot),
            "standards_snapshot": (target_run.standards_snapshot),
        }

        return LoadCalculationRunComparisonResponse(
            calculation_key=base_run.calculation_key,
            base_run_id=base_run.id,
            base_revision_number=(base_run.revision_number),
            target_run_id=target_run.id,
            target_revision_number=(target_run.revision_number),
    ...
    def _ensure_mutable(
        ...
        if calculation_run.is_immutable:
            raise ValueError("approved calculation runs are immutable")

        if calculation_run.approval_status == CalculationApprovalStatus.APPROVED.value:
            raise ValueError("approved calculation runs are immutable")
    ...
    def _build_content_hash(
        ...
        hash_payload = {
            ...
            "warnings_snapshot": (payload.warnings_snapshot),
            "standards_snapshot": (payload.standards_snapshot),
        }
    ...
    def _compare_values(
        ...
        *,
        ...
        if isinstance(base_value, dict) and isinstance(target_value, dict):
            keys = sorted(set(base_value) | set(target_value))
