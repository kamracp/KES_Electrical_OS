...
| P3 Org / RBAC / design basis | **Identity and access live** (EOS-01 a, 2026-09-20, `202a2ed`): organizations, users, roles OWNER / ENGINEER / VIEWER, server-side sessions, every engineering route behind sign-in; **project spine live** (EOS-01 b, 2026-09-26, `f22a71f`): sites, projects, revisions, runs linked to the open revision | jurisdiction profile now selectable per cable study (`jurisdiction_profile`, default IN); the project-level design-basis field is still to come (GAP-004, GAP-012) |
| P4 Load and demand | **Live** (EOS-02, 2026-09-26, `f22a71f`) | runs in the generic `calculation_runs` (`LOAD_DEMAND`), project scope, `/load-demand` page; old `/electrical/calculation-runs` path retired (`f22a71f`, table kept) |
| P5 Sources (Tx/DG/UPS/PV) | **Transformer live** (EOS-03a, 2026-09-26, `f22a71f`); DG / UPS / PV backend only | next: EOS-03b DG |
...
| Date | Commit | Notes |
|---|---|---|
| 2026-09-26 | `f22a71f` | Combined release EOS-01 (b) project spine + EOS-02 Load and demand + EOS-03a Transformer, per `docs/operations/release-2026-09-eos01b-eos02-eos03a.md`. Server before: `c9b99c0`, alembic `c9f5a3b7d2e4`; `load_calculation_runs` 0 rows, so EOS-02 commit 7 `f22a71f` retired the old `/electrical/calculation-runs` path. Backup `kes-electrical-os-20260926T125404Z-before-eos02-eos03a.dump` (104292 bytes, 13 tables, alembic `c9f5a3b7d2e4`); restore test in a scratch database: 19 runs. Migrations `c9f5a3b7d2e4` -> `d2a8b6c4e1f9` -> `e3b9d7f5a2c6`; live bundle `index-D-xiAMeU.js` equal to the build; `POST /load-demand/runs` and `/transformer-sizing/runs` answer 401 without a session. Backend 1298 / frontend 661 tests. Live smoke PASSED 2026-09-26 (founder, browser): EOS-01 (b), EOS-02 and EOS-03a as in the close entries under Active slice. |
...
- EOS-01 (b) live smoke — deferred 2026-09-23; DONE 2026-09-26 (release `f22a71f`, on PRJ-002).
- 15c PLANT-1 shown Inactive — NOT A DEFECT (code checked at `c9b99c0`: model default True + `server_default`
  ...
  confirmation before "Switch off". DONE `f466fcc`; Issue revision and Archive project ask the same way, DONE
  `5f1a6d5`; all three released `f22a71f`.
- `CLAUDE.md`: never run Prettier in this repo (no Prettier config; it rewrites whole files). DONE `4863077`.

From the 2026-09-26 release (new):

- PRIORITY - Study form drafts survive a reload and navigation: the founder lost a filled form twice during the
  2026-09-26 smoke. IN PROGRESS (Active slice, 2026-09-26). Related: F12 (session end while a form is half filled).
- A site with the code "PRJ-002" was created on live by mistake during the smoke; founder to decide whether
  to switch it off.
- Move `CalculationApprovalStatus` and `CalculationRunStatus` from the retired `models/load_calculation_run.py`
  into `models/calculation_run.py`; live code must not depend on a retired module (`f22a71f` commit body).
- Cable: size schedule as separate entries.
- Fault form: focus the new row after Add.
- Fault rows: guards against row label drift.
...
- OPERATING RULE until EOS-01 (b) is released (F2): exactly one organization on the live system - runs, the
  ...
  ENDED with the 2026-09-26 release of EOS-01 (b) (`f22a71f`).
...
- Module registry: EOS-01 turns LIVE with the project configuration page of (b), not before. DONE - EOS-01 is
  LIVE since the 2026-09-26 release (`f22a71f`).
...
4. **EOS-01 Project Configuration (order per A13):** (a) ~~identity and access~~ DONE - released `202a2ed`
   ...
   (b) ~~project spine (item 17)~~ DONE - released `f22a71f`, live smoke 2026-09-26.
5. ~~**EOS-02 Load and demand**~~ DONE and ~~**EOS-03a Transformer**~~ DONE - released `f22a71f` (2026-09-26).
6. **In progress:** the PRIORITY follow-up (study form drafts survive reload and navigation); then **EOS-03b DG**.
7. Then EOS-03 UPS / PV, EOS-05, EOS-07, EOS-08 … in §9 order; item 18 docs batch and item 19 §22 gate review
