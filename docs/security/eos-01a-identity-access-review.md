...
| 6 | **F5 CONFIRMED.** The page (`/`) carries none of `Strict-Transport-Security`, `X-Content-Type-Options`, `X-Frame-Options`, `Content-Security-Policy`, `Referrer-Policy`; the API (`/api/v1/health`) carries `X-Content-Type-Options`, `X-Frame-Options` and `Referrer-Policy`. First follow-up after the close. **Fixed 2026-09-22:** snippet `deployment/nginx/snippets/kes-security-headers.conf` included at server level and inside `location /` and `location /assets/`, installed by `scripts/deploy_nginx.sh` (`nginx -t` ok, reload, backup stamp 20260922-103147). After: `/` and `/api/v1/health` both carry all seven headers (`Strict-Transport-Security`, `X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`, `Permissions-Policy`, `Cross-Origin-Opener-Policy`, `Content-Security-Policy`); the policy allows only self, Google Fonts and the Cloudflare Web Analytics beacon; browser console clean on the fault study. |
