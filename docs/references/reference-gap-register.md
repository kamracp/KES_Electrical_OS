...
| GAP-014 | REF-IEC-60909, jurisdiction profiles | Profile-derived references (fault) | Fault engine `standard_reference` is a request default `IEC 60909-0:2026`; the edition year is not backed by any register row, and non-IN/IEC profiles would echo it exactly as GAP-013 did for cable | EOS-04 fault contract, `fault_models.py`, `fault_results.py`, `fault_engine.py` | Yes (non-IN/IEC profiles) | Resolve the IEC 60909-0 edition in the register; add a fault entry to `GoverningReferences`; apply the Cable resolution pattern (profile-first, override as deviation, `reference_source`) during the Fault UI slice | OPEN |
