...
| GAP-015 | (not yet registered) advisory limits for low power factor and low efficiency on load schedules | Unreferenced limit | No referenced source for the 0.80 limits previously hard-coded in `backend/app/domain/electrical/loads/engine.py` (`LOW_POWER_FACTOR_LIMIT`, `LOW_EFFICIENCY_LIMIT`) | EOS-02 `loads` engine | No (limits removed, warnings withdrawn) | Find a controlled reference per jurisdiction profile, then reintroduce the limits as profile-scoped values | OPEN |
...
| GAP-013 | Cable references are profile-derived: `GoverningReferences` on IN/IEC profiles (others carry none), engine resolves profile-first, request references are optional overrides reported as `REQUEST_OVERRIDE` deviations, `reference_source` on the contract, References panel shows "Reference pending" for unresolved profiles | `03bd578`..`a2797ee` | 2026-09-17 |
| GAP-014 | Fault references are profile-derived: `FaultGoverningReferences` (`IEC 60909-0` / `IEC 60909-3`, no edition years) on IN/IEC profiles (others carry none); `ShortCircuitStudyInput` gained `jurisdiction_profile`; engine resolves profile-first, request references are optional overrides reported as `REQUEST_OVERRIDE` deviations; API and frontend carry `reference_source`, profile and verification status; the unbacked `IEC 60909-0:2026` / `IEC 60909-3:2009` defaults are gone. IEC 60909 editions remain unresolved (GAP-005 pattern) | `3b94075`..`e2338a6` | 2026-09-17 |
