...
| A10 | Shell navigation: one Back control in the shell topbar on every page except Home — browser-history back, falling back to Home when there is no in-app history; implemented once in the shell, never per page; an unsaved-draft warning is deferred until EOS-01 drafts exist | Founder requirement 18 Sep 2026 after EOS-06 close `5304297` |
| A11 | Item 15 clarified — idempotent runs: a Calculate whose content hash (input + result) and engine version both equal those of the latest revision for the same module and study code returns that existing run instead of creating a new revision; a new revision is created only when the evidence or the engine changes. Implemented once in the generic run service, for every module | Founder decision 18 Sep 2026 after live smoke SC-MSB-01 (revisions 1, 2, 3 … with one identical hash) |
| A12 | §4/§19 display rule: result summaries and tables show engineering figures to 4 significant figures with their unit; the exact engine decimal stays available on the page (tooltip) and is never altered in the persisted run or the JSON export; one shared formatter for every module | Founder decision 18 Sep 2026 (summary showed `35.219668322 kA`) |
| A13 | §15 execution order amended - access control before project data: EOS-01 is built in two parts, (a) identity and access (organizations, users, roles, login with a server-side session, every engineering API behind authentication) and then (b) the item 17 project spine; until (a) is released no organization, site, project or client name is stored on the live system; the (a) slice gets an independent review before release | Founder decision 19 Sep 2026 (the live site has no login; project names would be public) |
| A14 | EOS-01 (b) project spine: existing calculation runs stay unassigned (no data migration); calculating without a project remains allowed and creates an unassigned run; a chosen project links the run to that project's open revision. Load calculation runs are not linked in this slice. | Founder delegated 2026-09-22 ("no preference" on both questions); recommendation adopted |
| A15 | EOS-02 Load and demand rules: (a) a blank utilization, demand, efficiency or coincidence factor means NOT ESTABLISHED — the engine proceeds with 1, raises a named not-established warning and sets `REVIEW_REQUIRED`, never a silent default; (b) the unreferenced 0.80 low power-factor and low-efficiency limits are removed until a reference exists (GAP-015); (c) load studies carry a jurisdiction profile and, when a project revision is chosen, must match the project's profile (409 otherwise), as Cable and Fault; (d) load runs are stored in the generic `calculation_runs` table (A11 reuse, project revision link, organization scope, traceability); the old `load_calculation_runs` table is kept, its routes are retired; (e) the coincidence factor is applied equally to P and Q — a declared assumption shown in the result and traceability; (f) approval UI and four-eyes stay in the separate approvals slice (F14) | Founder delegation 2026-09-23; EOS-02 inspection at `feb5934` |
| A16 | EOS-03 source sizing: (a) the A15 not-established rule applies - a blank design margin, growth or derating factor is NOT ESTABLISHED: calculated as 1, named warning, `REVIEW_REQUIRED`; no silent defaults (the invented 1.10 / 1.20 margins and 0.94 / 0.97 / 0.98 efficiencies are removed); (b) UPS battery runtime is a required design-basis input, never defaulted; (c) unreferenced loading and DC/AC ratio warning limits are removed (GAP-016); (d) one status vocabulary for all source engines: `VALID`, `WARNING`, `REVIEW_REQUIRED`, `NO_SOLUTION`; (e) standard ratings keep coming from the request (project design basis); (f) EOS-03 is delivered in three parts, each released and closed: 03a Transformer, 03b DG, 03c UPS + PV | Founder delegation 2026-09-23; EOS-03 inspection at `c25ae0e` |
...
**Shell back control (A10):** executed after EOS-06 Cable closes and before EOS-04 Fault starts, as one small shell slice: `BackButton` component with tests, wiring into the shell topbar, shell styles, release and live smoke. It is shell work with the same standing as item 16a and does not change the module order above.

**Idempotent runs (A11):** executed immediately after EOS-04 (a)+(b) close and before EOS-04 (c) Fault UI v2, as one small backend slice in the generic run service with tests, release and a live probe. It changes when a revision is created, never what a stored run contains.

**Display precision (A12):** executed together with EOS-04 (c) Fault UI v2, as one shared frontend formatter applied to the Cable and Fault summaries and tables; stored evidence and exports are untouched.
...
17. **P3 minimal project spine (EOS-01).** *(A13: this item is EOS-01 (b); EOS-01 (a) identity and access - users, roles, login - is built and released first.)* Organization, site, project, revision; jurisdiction profile held on the project; Cable and Fault runs linked to a project revision. UI: project selector in the shell topbar.
